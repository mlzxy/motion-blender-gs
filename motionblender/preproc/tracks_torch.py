...
import numpy as np
import torch
from motionblender.preproc.tapnet_torch import tapir_model, transforms
from tqdm import tqdm


def read_video(folder_path, frame_names):
    frame_set = set([os.path.basename(f).split('.')[0] for f in frame_names])
    frame_paths = sorted(glob.glob(os.path.join(folder_path, "*")))
    frame_paths = [f for f in frame_paths if os.path.basename(f).split('.')[0] in frame_set]
...
def main():
    ...
    parser.add_argument("--image_prefix", type=str, default="", help="image prefix")
    parser.add_argument("--mask_dir", type=str, required=True, help="mask dir")
    parser.add_argument("--dataset-json", type=str, default="", help="dataset json")
    parser.add_argument("--names_json", type=str, default="", help="name json (instance types)")
    ...
    parser.add_argument("--target_inst", type=int, default=-1)
    parser.add_argument("--model_type", type=str, choices=["tapir", "bootstapir"], help="model type")
    parser.add_argument(
        "--ckpt_dir",
        type=str,
        default="checkpoints",
    ...
    if args.dataset_json:
        print(f"loading train frame names from {args.dataset_json}")
        dataset_json = json.loads(Path(args.dataset_json).read_text())
        if 'train_ids' in dataset_json:
            frame_names = dataset_json['train_ids']
        else:
            frame_names = dataset_json['ids']
        frame_names = [f + '.png' for f in sorted(frame_names)]
    else:
        frame_names = [os.path.basename(f) for f in sorted(glob.glob(os.path.join(folder_path, "*")))]
    frame_names = [f for f in frame_names if f.startswith(args.image_prefix)]
    
    print(f"num frames: {len(frame_names)} with prefix `{args.image_prefix}`")
    ...
    video = read_video(folder_path, frame_names)
    num_frames, height, width = video.shape[0:3]
    masks = read_video(mask_dir, frame_names)
    if args.names_json:
        name_json = json.loads(Path(args.names_json).read_text())
        for name_i, name in enumerate(name_json):
            if name.endswith(":static"):
                print(f"removing static mask {name}={name_i}")
                masks[masks == (name_i + 1)] = 0
    if args.target_inst > 0:
        masks = masks == args.target_inst
    masks = (masks.reshape((num_frames, height, width, -1)) > 0).any(axis=-1) # 0 is static background
