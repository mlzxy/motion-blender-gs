...
if TYPE_CHECKING:
    from nerfview.viewer import CameraState, Viewer
        ...
        while self.running:
            ...
            try:
                with self.lock, set_trace_context(self._may_interrupt_trace):
                    ...
                    if task.message: 
                        guru.info(f"Rendering from task: {task.message}")
                    rendered = self.viewer.render_fn(task.camera_state, img_wh, cache_refresh=self._cache_refresh, buffer=task.buffer)
                    if self._cache_refresh: self._cache_refresh = False
