...
class iPhoneDataset(BaseDataset):
    def __init__(
        ...
        **_,
        ...
        if osp.exists(osp.join(self.data_dir, "flow3d_preprocessed/track_anything/")):
            has_masks = True
        else:
            has_masks = False
        ...
        if not skip_load_imgs and has_masks:
                ...
                def load_depth(frame_name):
                    if self.depth_type == "lidar":
                        ...
                        depth = np.load(
                            osp.join(
                                self.data_dir,
                                f"flow3d_preprocessed/aligned_{self.depth_type}/", # depth_anything_colmap
        ...
        if has_masks:
            ...
            self.depths = torch.zeros(self.imgs.shape[:3])
    ...
    def get_tracks_3d(
        ...
        for i in (
            ...
            for j in range(0, self.num_frames, step):
                if i == j:
                    ...
                    track_path = f"flow3d_preprocessed/2d_tracks/{self.factor}x/"
                    if not osp.exists(osp.join(self.data_dir, track_path)): # try general path
                        track_path = f"bootstapir/{self.img_path_suffix}/"
        ...
        filtered_tracks_3d = torch.cat(filtered_tracks_3d, dim=1).swapdims(0, 1) # all_pts, frames, 3
    ...
    def get_bkgd_points(
        ...
        **kwargs
        ...
        candidate_frames = list(range(len(self.imgs)))
        num_sampled_frames = len(candidate_frames)
        bkgd_points, bkgd_point_normals, bkgd_point_colors = [], [], []
        tqdm_tag = 'bkgd'
        if instance != 'bg':
            tqdm_tag = f"instance {instance}"
        for i in tqdm(candidate_frames, desc=f"Loading {tqdm_tag} points"):
            img = self.imgs[i]
            depth = self.depths[i]
            if instance == 'bg':
                _mask = 1.0 - self.masks[i]
            else:
                _mask = (self.instance_masks[i] == instance).float()
            bool_mask = (_mask * self.valid_masks[i] * (depth > 0)).to(
                torch.bool
            )
            w2c = self.w2cs[i]
            K = self.Ks[i]
...
class iPhoneDatasetVideoView(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try: 
            result['depths'] = self.dataset.depths[index]
        except: 
            pass
        try: 
            result['instance_masks'] = self.dataset.instance_masks[index]
        except: 
            pass
        result["cam_name"] = self.img_prefix
        return result
